...
With the growth of the so-called "*quantified self*" movement, there has been an increased demand for data collection tools (and computational strategies for analyzing said data) relating to personal wellness. However, while much research has been dedicated to the purely quantitative measurements--for instance, counting steps taken or calories burned--very little research has been focused on the *qualitative* aspects of movement and exercise. In other words, while many reasearchers have analyzed "*how much*", few have sought to describe "*how well*". It's with this basic premise in mind that we proceed with our current machine learning project: An attempt at using aggregated sensor data in order to identify and classify the *quality* of certain exercises performed [Note: For more information regarding the actual experiment performed, please see the "Citations" section of this report]. 

### Downloading the Necessary Data

Before we can begin our analysis, we first have to load the necessary packages as well as actually *assemble* our data. This latter step can often be the most difficult for a data scientist. Fortunately, the original researchers have graciously allowed us access to their dataset, greatly facilitating this often time-consuming step:
...
As is evident from the **Confusion Matrix**, our results are actually very good despite the simplicity of our model.  Since **random forests** do tend to run the risk of *over-fitting*, it's important that we can ascertain an appropriate estimate of the out-of-sample error rate. Although the **Out-of-Bag** estimate of the error rate has been shown in tests of random forests to be an unbiased estimator of the error rate, we choose the error rate achieved for the **cross-validation** set as our estimate of out-of-sample error. Since the **cross validation** set was completely untouched while growing our random forest (after all, we only grew a *single* forest), this error rate will serve as a completely unbiased estimate of the actual (unknown) out-of-sample rate. Nevertheless, the **OOB** estimate of the error rate gives us a glimpse of order of magnitude that we should expect. Indeed, as the results show, the estimate obtained via the **cross-validation** set is within only a few basis points of our **OOB** estimate.
